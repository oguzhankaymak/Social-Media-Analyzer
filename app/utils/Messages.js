export default {
  //general
  success: 'Başarılı!',
  generalErrorTitle: 'Maalesef İşleminiz Gerçekleştirilemedi!',
  generalErrorContent: 'Bir sorun oluştu lütfen daha sonra tekrar deneyiniz.',
  okay: 'Tamam',
  pleaseAttention: 'Lütfen Dikkat!',

  //login - register
  registerSuccess: 'Kayıt olma işlemini başarılı bir şekilde gerçekleştirdiniz. Giriş yapabilirsiniz.',
  invalidNameSurname: 'Lütfen geçerli ad soyad giriniz.',
  invalidEmail: 'Lütfen geçerli bir email adresi giriniz.',
  invalidPassword: 'Lütfen şifrenizi doğru giriniz.',
  invalidPasswordConfrim: 'Lütfen şifre doğrulamanızı doğru giriniz.',

  //account
  noChanges: 'Değişiklik gerçekleştirmediniz.',
  successfullyChanged: 'Bilgileriniz başarılı bir şekilde güncellenmiştir.',
  galleryPermission: 'Profil fotoğrafınızı yükleyebilmeniz için izin vermelisiniz.',
  tryAgain: 'Tekar Dene',
  changesWillBeLost: 'Değişiklikleriniz kaybolacaktır.',
  renounce: 'Vazgeç',
  photoSizeError:
    'Lütfen daha düşük boyutlu fotoğraf seçiniz. Ya da fotoğrafınızı seçerken yüksekliğini ve genişliğini düşürebilirsiniz.',
};
