...
import styles from './styles/NotToBeFollowedStyle';
import UserList from '../../../components/userList/UserList';
import { CalendarIcon, SearchIcon } from '../../../components/icons';
import { Colors } from '../../../theme';

const NotToBeFollowed = ({ route }) => {
  const [username, setusername] = useState('');
  const [data, setdata] = useState(route?.params?.notToBeFollowedUsers);
  const [filterData, setFilterData] = useState(route?.params?.notToBeFollowedUsers);
...
export default NotToBeFollowed;
